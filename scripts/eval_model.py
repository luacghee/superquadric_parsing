#!/usr/bin/env python
"""Script used to evaluate the Chamfer distance and IOU on a set of pre-trained models
"""
import argparse
import os
import sys
...
def main(argv):
    ...
    for sample in dataloader:
        ...
        # Compute volume IOU
        use_sq = get_loss_options(args).get("use_sq", False)
        iou = eval_iou(y_target, y_hat, use_sq)
        ious.append(iou)
    ...
    np.savetxt(
        os.path.join(args.output_directory, "ious.txt"),
        ious
    )
